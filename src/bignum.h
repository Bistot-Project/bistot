...
// Copyright (c) 2018-2019 The Bistot Developers
...
#else
        unsigned long n = BN_get_word(this->bn);
...
#else
        unsigned int nSize = BN_bn2mpi(this->bn, NULL);
#endif
        if (nSize < 4)
            return 0;
...
#else
        BN_mpi2bn(&vch2[0], vch2.size(), this->bn);
#endif
    }
...
#else
        BN_mpi2bn(&vch[0], vch.size(), this->bn);
#endif
        return *this;
    }
...
#else
        if (!BN_exp(ret.bn, this->bn, e.bn, pctx))
#endif
...
#else
        if (!BN_mod_mul(ret.bn, this->bn, b.bn, m.bn, pctx))
#endif
...
#else
            if (!BN_mod_exp(ret.bn, this->bn, e.bn, m.bn, pctx))
#endif
...
#else
        if (!BN_mod_inverse(ret.bn, this->bn, m.bn, pctx))
#endif
...
#else
        if (!BN_gcd(ret.bn, this->bn, b.bn, pctx))
#endif
...
#else
        int ret = BN_is_prime_ex(this->bn, checks, pctx, NULL);
#endif
        if(ret < 0){
...
#else
        if (!BN_add(this->bn, this->bn, b.bn))
#endif
...
#else
        if (!BN_mul(this->bn, this->bn, b.bn, pctx))
#endif
...
#else
        if (!BN_lshift(this->bn, this->bn, shift))
#endif
...
#else
        if (!BN_rshift(this->bn, this->bn, shift))
#endif
...
#else
        if (!BN_add(this->bn, this->bn, BN_value_one()))
#endif
            throw bignum_error("CBigNum::operator++ : BN_add failed");
        return *this;
    }
...
#else
        if (!BN_sub(r.bn, this->bn, BN_value_one()))
#endif
